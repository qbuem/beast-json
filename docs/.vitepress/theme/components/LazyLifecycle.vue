...
const phases: Phase[] = [
  {
    ...
    icon: '⚡',
    steps: [
      'SIMD Stage 1: load 64 bytes into a ZMM register',
      'cmpeq / cmpgt masks → structural __mmask64',
      'prefix_xor shift-ladder suppresses in-string chars',
      'Stage 2: ctzll over the mask, write TapeNodes',
  ...
  },
  {
    ...
    icon: '🔍',
    steps: [
      'root["user"]  →  scan tape StringRaw keys',
      'skip_value_ streams over subtrees you skip',
      'Returns Value{ doc*, idx=2 }',
      'No data extracted — only a 12-byte cursor',
      'Repeat navigation stays fully cache-hot',
    ],
    output: 'Value{ doc*, idx }',
    outputLabel: '12-byte cursor — no heap, no copy',
  },
  {
    num: 3,
    title: 'EXTRACT',
    ...
    steps: [
      ...
      '.as<int>()  →  from_chars over the source slice',
      'One slice read per field — zero allocation',
