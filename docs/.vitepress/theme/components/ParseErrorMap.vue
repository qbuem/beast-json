...
const scenarios: ErrorScenario[] = [
  // ── PARSE ERRORS ─────────────────────────────────────────────────
  {
    ...
    message: 'beast::parse_error: unterminated string literal at byte 10',
  ...
  },
  {
    ...
    message: 'beast::parse_error: unexpected token \'}\' after comma at byte 18',
  ...
  },
  {
    ...
    message: 'beast::parse_error: invalid escape sequence \'\\q\' at byte 22',
  ...
  },
  {
    ...
    message: 'beast::parse_error: integer overflow — value exceeds 2^56 at byte 9',
    why: 'Beast JSON\'s INT64/UINT64 payload is 56 bits (the remaining bits after the 8-bit type tag). Values > 2^56-1 cannot be stored inline. The Russ Cox algorithm detects overflow during accumulation.',
  ...
  },
  {
    ...
    message: 'beast::parse_error: unexpected end of input — unclosed object at byte 7',
  ...
  // ── TYPE ERRORS ───────────────────────────────────────────────────
  {
    ...
    message: 'beast::type_error: expected INT64/UINT64, got STRING at tape[2]',
  ...
  },
  {
    ...
    message: 'beast::access_error: key "missing" not found in object',
  ...
  // ── LIFETIME ERRORS ───────────────────────────────────────────────
  {
    ...
    why: 'string_view from Beast JSON points directly into the input buffer. When the input buffer (json std::string) is destroyed, the string_view\'s pointer becomes dangling. This is silent UB — AddressSanitizer will catch it.',
  ...
  },
  {
    ...
    category: 'lifetime',
    level: 'intermediate',
    input: 'beast::Value get_value() {\n    beast::Document doc;\n    auto root = beast::parse(doc, input);\n    return root["user"];  // doc is destroyed on return!\n}',
    errorMark: 'doc is destroyed',
    stage: 'Runtime — undefined behaviour',
    message: '(no exception — Value holds a dangling doc* pointer)',
    why: 'beast::Value stores a pointer to its DocumentView (doc_). When Document is destroyed, all Values referencing it become dangling. Any subsequent operation on the returned Value is UB.',
  ...
  // ── SERIALIZE ERRORS ─────────────────────────────────────────────
  {
    ...
    category: 'serialize',
    level: 'intermediate',
    input: 'char buf[16];\nbeast::write_to(buf, 16, value);  // value is large',
    errorMark: 'buf[16]',
    stage: 'Runtime — Serializer::write_to()',
    message: 'beast::serialize_error: output buffer capacity exceeded',
